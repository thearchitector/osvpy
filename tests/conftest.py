...
@pytest.fixture
def report(recorded_scan: dict[str, "Any"]) -> osvpy.FullScanResult:
    ...
    return osvpy.FullScanResult.model_validate({
        "image": "ubuntu:latest",
...
@pytest.fixture
def finding_factory(
    report: osvpy.FullScanResult,
) -> "Callable[..., osvpy.Vulnerability]":
    data = report.vulnerabilities[0].model_dump(exclude_computed_fields=True)

    def make(**advisory_fields: object) -> osvpy.Vulnerability:
        return osvpy.Vulnerability.model_validate({
            **data,
...
@pytest.fixture
def offline_scan(archive_pair: tuple[Path, Path]) -> "Callable[..., osvpy.ScanResult]":
...
@pytest.fixture
def advisory_scan(
    archive_pair: tuple[Path, Path], offline_scan: "Callable[..., osvpy.ScanResult]"
) -> "Callable[..., osvpy.ScanResult]":
    ...
    def scan(*advisories: dict[str, "Any"]) -> osvpy.ScanResult:
