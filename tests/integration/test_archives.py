...
def test_offline_scan_finds_vulnerability_in_unicode_path(
    ...
    assert vulnerability.id == "OSVPY-TEST-0001"
...
def test_all_packages_controls_inventory(
    offline_scan: "Callable[..., osvpy.ScanResult]", all_packages: bool, names: set[str]
) -> None:
...
def test_full_details_are_opt_in(
    ...
    assert isinstance(result, osvpy.FullScanResult)
...
def test_invalid_report_detail(offline_scan: "Callable[..., osvpy.ScanResult]") -> None:
...
def test_offline_license_check_is_not_silently_skipped(
    ...
    with pytest.raises(
        osvpy.OfflineDatabaseError, match="License checks require online"
    ):
        offline_scan(allowed_licenses={"MIT"})


def test_scan_is_quiet(
    offline_scan: "Callable[..., osvpy.ScanResult]", capfd: pytest.CaptureFixture[str]
) -> None:
    offline_scan()
...
@pytest.mark.parametrize(
    ("name", "error"),
    [("missing", osvpy.ImageNotFoundError), ("invalid", osvpy.ScanError)],
)
def test_unreadable_archive(
    bad_archives: dict[str, Path], name: str, error: type[osvpy.OSVError]
) -> None:
    with pytest.raises(error):
        osvpy.scan_docker_archive(bad_archives[name])
