...
class Vulnerability(BaseModel):
    ...
    def fixed_version(self) -> str | None:
        fixes = self.fixed_versions
        return fixes[0] if len(fixes) == 1 else None
    ...
    def severity(self) -> float | None:
        ...
        for severity in self.advisory.severity or []:
            parser = {"CVSS_V2": CVSS2, "CVSS_V3": CVSS3, "CVSS_V4": CVSS4}.get(
                severity.type or ""
            )
            if parser is not None and severity.score is not None:
                with suppress(CVSSError):
                    scores.append(float(parser(severity.score).scores()[0]))
